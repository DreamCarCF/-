...
{
    return [self getFilterPerson:nil];
}




-(NSMutableArray*)getFilterPerson:(NSArray *)blackItemArr
{
    ...
    for (NSInteger i = 0; i < nPeople; i++)
    {
        ...
        if(blackItemArr != nil){
            BOOL isExistBlackItemArr = NO;
            for(NSUInteger i=0; i < blackItemArr.count; i++){
                if([[blackItemArr objectAtIndex:i] isEqualToString:addressBook.tel]){
                    isExistBlackItemArr = YES;
                    break;
                }
            }
            
            if(isExistBlackItemArr == NO){ //当前电话号码不存在于黑名单列表中
                continue;
            }
    ...
    for (NSMutableArray *sectionArray in sectionArrays) {
        ...
        PersonModel *person2=(PersonModel*)sectionArray.lastObject;
        if(person.name1==nil||[person.name1 isEqualToString:@""]||person2.name1 == nil || [person2.name1 isEqualToString:@""])
        {
            continue;
        }
